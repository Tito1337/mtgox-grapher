...
function plot() {
    ...
    debug((realMax(maximum)-minimum)/60);
    isRedrawing = true;
    $.plot($("#trades"), [{data:showVolumes, color:1, bars:{show: true, barWidth:((realMax(maximum)-minimum)/40)}, yaxis:2},
...
function onOpen(evt) {
    ...
    websocket.send('{"op":"mtgox.subscribe","key":"6feefa9c-633b-45a0-899a-195f99a65a60:4pMe4y85ldbkku6kkSHiZN20CI8="}');
}
...
$(function () {
    if ("WebSocket" in window) {
        $.get('getFalseTrades.php', function(data) {
            trades = eval('(' + data + ')');
            plot();
            startWebsocket();
            refresh();
